...
namespace XGraphics.SvgImporter
{
	...
	{
		private static Dictionary<string, string>? _colorNamesToHexValues;
		...
		public static bool TryParse(string str, out Color color)
		{
            ...
            else if (ColorNamesToHexValues.TryGetValue(str, out string hexString))
                str = hexString;

            if (str.StartsWith("#", StringComparison.Ordinal) && Color.FromHex(str, out color))
                return true;
		...
		public static Dictionary<string, string> ColorNamesToHexValues
		{
			get
			{
				if (_colorNamesToHexValues == null)
				{
					_colorNamesToHexValues = new Dictionary<string, string>
					{
						{ "aliceblue", "#f0f8ff" },
						{ "antiquewhite", "#faebd7" },
						{ "aqua", "#00ffff" },
