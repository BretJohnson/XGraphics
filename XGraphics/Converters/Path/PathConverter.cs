...
namespace XGraphics.Converters.Path
{
    public static class PathConverter
    {
        ...
        /// F1 is NonZero).
        /// </summary>
        public static IPathGeometry ParsePathGeometry(string pathString, IGeometryFactory geometryFactory)
        {
            PathStreamGeometryContext context = new PathStreamGeometryContext(geometryFactory);

            FillRule fillRule = FillRule.EvenOdd;
            ParseStringToStreamGeometryContext(context, pathString, ref fillRule);

            return context.FinishAndGetPathGeometry(fillRule, null);
        }
        ...
        /// </summary>
        private sealed class AbbreviatedGeometryParser
        {
            static readonly string UnexpectedTokenMessage = "Unexpected token '{0}' encountered at position '{1}'.";
            ...
            readonly IFormatProvider _formatProvider = System.Globalization.CultureInfo.InvariantCulture;

            readonly string _pathString;        // Input string to be parsed
            ...
            public AbbreviatedGeometryParser(string pathString)
            {
            ...
            {
                throw new FormatException(string.Format(UnexpectedTokenMessage, _pathString, _curIndex - 1));
            }

            private bool More() => _curIndex < _pathLength;
            ...
            double ReadNumber(bool allowComma)
            {
                ...
                else
                {
                    ...
                    {
                        throw new FormatException(string.Format(UnexpectedTokenMessage, _pathString, start), except);
                    }
                }
            }
            ...
            /// </summary>
            internal void ParseToGeometryContext(StreamGeometryContext context, int startIndex)
            {
                _context = context;
                    ...
                    switch (cmd)
                    {
                        ...
                        case 'V':
                            ...
                            do
                            {
                                switch (cmd)
                                {
                                    ...
                                    case 'L': _lastPoint = ReadPoint(cmd, !AllowComma); break;
                                    case 'h': _lastPoint = _lastPoint.WithX(_lastPoint.X + ReadNumber(!AllowComma)); break;
                                    case 'H': _lastPoint = _lastPoint.WithX(ReadNumber(!AllowComma)); break;
                                    case 'v': _lastPoint = _lastPoint.WithY(_lastPoint.Y + ReadNumber(!AllowComma)); break;
                                    case 'V': _lastPoint = _lastPoint.WithY(ReadNumber(!AllowComma)); break;
                                }
                        ...
                        case 'S': // smooth cublic Bezier
                            ...
                            do
                            {
                                ...
                                context.BezierTo(p, _secondLastPoint, _lastPoint);
                        ...
                        case 'T': // smooth quadratic Bezier
                            ...
                            do
                            {
                                ...
                                context.QuadraticBezierTo(_secondLastPoint, _lastPoint);
